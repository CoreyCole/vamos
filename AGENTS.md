...
Use authored Go Story API tests under `pkg/e2e/tests` as the canonical E2E specs. Use `vamos e2e run` only in a registered non-main workspace when fixtures are enabled; it refuses canonical main DBs. The old markdown story parser/generator path has been removed; do not add `docs/features/*.story.md` or `pkg/e2e/generated`. Visual review belongs to `vamos e2e review` and `e2e-image-review`, not deterministic test runs. `vamos e2e fix` is bounded to selectors, steps, runtime, authored Go Story tests, and Vamos E2E helpers unless a human explicitly approves product changes.
