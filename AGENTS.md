...
For authenticated browser or human testing, use the short-lived manager-minted token flow in `docs/workspaces-verification.md` (Commands) or `docs/e2e-story-testing.md` (Auth for public workspace E2E). Create or load a machine credential, mint the token with `vamos auth playwright-env`, and open `/internal/agent-auth/browser-login`; do not use the legacy `/internal/playwright-auth` shortcut. Use the target server's configured workspace slug—verify it from runtime diagnostics or the `server_startup` log instead of inferring it from the port, checkout, or binary path. Build the browser-login URL on the exact origin the human will test because the resulting session cookie is host-scoped; never substitute `localhost` or `127.0.0.1` for a Tailnet or workspace host. Validate that the fresh login URL returns HTTP 307 and then open it directly in the local browser; do not paste the bearer token into the chat transcript. On ruby, use `vamos-va-mint list` / `vamos-va-mint cookie` (defaults to the active todo52 feature host) so the profile matches AllowedSlugs; see `docs/e2e-story-testing.md` (Ruby VA profile → slug map). Write session artifacts mode 600 under `/tmp` and never paste cookies/tokens into chat.
