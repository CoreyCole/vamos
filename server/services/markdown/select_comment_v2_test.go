...
func newSelectCommentV2Service(t *testing.T) (*Service, *dbsvc.Service) {
	t.Helper()
	root := t.TempDir()
	if err := os.MkdirAll(filepath.Join(root, "owner", "plan-a"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(
		filepath.Join(root, "owner", "plan-a", "AGENTS.md"),
		[]byte("# Workspace\n"),
		0o644,
	); err != nil {
		t.Fatal(err)
	...
	if err := os.WriteFile(
		filepath.Join(root, "owner", "plan-a", "design.md"),
		[]byte("# Design\n"),
		0o644,
	); err != nil {
		t.Fatal(err)
	...
	comments := commentsvc.NewService(
		database.DB(),
		"test-commit",
		"https://github.com/example/repo/blob",
		root,
	)
	service, err := NewService(root, comments, nil)
	if err != nil {
		t.Fatal(err)
	}
