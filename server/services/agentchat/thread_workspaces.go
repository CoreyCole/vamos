...
func (s *Service) GetThreadWorkspaceContext(
	ctx context.Context,
	userEmail, threadID string,
) (ThreadWorkspaceContext, error) {
...
func (s *Service) SetThreadPrimaryWorkspace(
	ctx context.Context,
	threadID, workspaceID, source string,
) error {
	...
	if err := q.UpsertThreadWorkspaceAssociation(
		ctx,
...
func (s *Service) AddThreadRelatedWorkspace(
	ctx context.Context,
	threadID, workspaceID, source string,
) error {
	return s.queries.UpsertThreadWorkspaceAssociation(
		ctx,
		db.UpsertThreadWorkspaceAssociationParams{
			ThreadID:    strings.TrimSpace(threadID),
			WorkspaceID: strings.TrimSpace(workspaceID),
...
func (s *Service) AttachThreadRunsAndSessionsToPrimary(
	ctx context.Context,
	threadID, workspaceID string,
) error {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return err
...
func (s *Service) ResolvePrimaryWorkspaceForThread(
	ctx context.Context,
	userEmail, threadID string,
) (db.Workspace, bool, error) {
	workspace, err := s.queries.GetPrimaryWorkspaceForThread(
		ctx,
...
func (s *Service) threadHasWorkspaceAssociation(
	ctx context.Context,
	threadID, workspaceID string,
) (bool, error) {
	threadID = strings.TrimSpace(threadID)
	...
	associated, err := s.queries.ThreadHasWorkspaceAssociation(
		ctx,
		db.ThreadHasWorkspaceAssociationParams{
			ThreadID:    threadID,
			WorkspaceID: workspaceID,
		},
	)
