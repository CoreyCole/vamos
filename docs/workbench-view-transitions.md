...
10. Minimal JS only (`static/js/workbench-history.js`): Enter/Up `pushState({ workbenchArtifactPatch })` + popstate reload gate so Back never blanks; pageshow `queueMicrotask` → `#chat-latest` focus (mirrors mobile Chat tab; fixes desktop pin after VT); Files cookie on templ button + SSR; history.js stays tiny.
...
- `static/js/workbench-history.js` — pushState patch flag + popstate reload gate + pageshow `#chat-latest` focus (no morph)
