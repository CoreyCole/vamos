...
10. Minimal JS only (`static/js/workbench-history.js`): Enter/Up `pushState({ workbenchArtifactPatch })` + popstate reload gate so Back never blanks; after `pagereveal` / `viewTransition.finished`, `pinChatToBottom` pins whichever of `#agent-chat-messages` / `#agent-chat-scroll-region` actually overflows (`scrollHeight > clientHeight`), then focuses `#chat-latest` with `preventScroll` (focus alone leaves a gap above the composer); Files cookie on templ button + SSR; history.js stays tiny.
