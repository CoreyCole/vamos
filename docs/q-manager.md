...
q-manager child sessions load a local Pi extension plus CLI validation loop. Pi `agent_settled`, not low-level `agent_end`, is the normal completion boundary. Interruption chat does not consume result-repair budget. The extension invokes Go `qrspi child-complete`; Go reads the active JSONL branch after the persisted evidence cursor, validates/normalizes the result, generates `validation-status.json`, and delivers or queues the wake. The extension may still write child `status.json` and touch `done` as diagnostics, but those files are not authoritative manager triggers.
