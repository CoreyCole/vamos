...
The project-local `.pi/extensions/q-manager-child` currently validates managed-child identity but installs no settlement behavior while the opaque settlement contract is being rebuilt. It carries the retained MIT attribution for patterns adapted from the pinned read-only interactive-subagents context; global `npm:pi-subagents` is not loaded. Vamos remains the authority for QRSPI results, graph, plan/workspace policy, managed/chat semantics, claims, and audit.
