...
Blocked/error/human stops stay short and preserve the child pane for inspection:
...
1. When the child finishes, confirm the parent pane receives one buffered wake prompt with `q_manager_child_wake` and `param: "vamos qrspi continue --state-file <state>"`.
