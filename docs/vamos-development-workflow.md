...
| `../vamos` | editable staging/local Vamos checkout | prototype/commit local-main work here; exposed as `work` workspace |
...
- `work.workspaces...` previews editable `../vamos` when main manager starts configured `work`.
...
- Routine staging/prototype work may happen in editable `../vamos` and commit to local `main`.
...
- Workspace `.vamos/state/agents.db` and `.vamos/state/temporal.db` are sandbox runtime state for `work` and feature URLs.
...
Use `main` to manage. Use `work` or feature URLs to test runtime/chat/Temporal UX safely. Promote only from main when explicit evidence exists: QRSPI human-review readiness, verification artifacts, passing tests, and release preflight.
