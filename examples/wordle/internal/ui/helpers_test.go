...
func TestTileAttrsAnimateOnlyMarkedTiles(t *testing.T) {
	t.Parallel()
	attrs := tileAttrs(
		TileView{Index: 2, State: stateCorrect, DelayMS: 200, Animation: "flip"},
	)
	...
	attrs = tileAttrs(TileView{Index: 2, State: stateCorrect})
...
func TestRowAttrsUseTransientAnimation(t *testing.T) {
	t.Parallel()
	attrs := rowAttrs(GuessRow{Current: true, Animation: animationShake})
	if got := attrs["data-wordle-row"]; got != "current" {
		t.Fatalf("row attr = %v, want current", got)
	}
	if got := attrs["data-wordle-animation"]; got != animationShake {
...
func TestKeyAttrsAnimateDelayedKeys(t *testing.T) {
	...
	if got := attrs["data-wordle-animation"]; got != animationKey {
