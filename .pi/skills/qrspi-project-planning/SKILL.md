...
- vertical milestone slices: one demonstrable product path/scenario at a time
...
- milestone taxonomy and sequencing, preferably vertical by product scenario/user path rather than horizontal system layer/capability area; milestone names should use product/domain names, not the word "Vertical"
...
- provider milestone/planning ticket links
- canonical pointers
...
- stable identity path under `tickets/[provider-key]-slug/`; do not prefix with sequence numbers because ticket order changes and provider IDs are stable
...
- milestone sequence with links to provider issue-list milestone views when milestones already exist; provider URLs come from the thoughts root `AGENTS.md` manifest
- one section/table per future milestone, using rows as milestone-planning inputs
- row columns such as `Proposed work | Critical-path outcome | Notes | Provider issue`
...
- source index linking canonical research, brainstorms, provider-ticket comments, and source docs
...
```yaml
qrspi_ticket_provider: linear
qrspi_ticket_provider_config:
  thoughts_viewer_base_url: "https://example.internal/thoughts"
  thoughts_viewer_path_pattern: "/thoughts/{path}"
  linear_base_url: "https://linear.app/example"
...
If provider config is missing or unsupported, create-tickets emits `needs_human` or `blocked` instead of mutating external issues.
...
- adding copied workspace/Graphite boilerplate to plans whose only outputs are project-planning docs, provider comments, milestone maps, or living specs
