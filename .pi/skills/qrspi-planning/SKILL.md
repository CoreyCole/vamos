...
- **Fresh implementation directories, never worktrees.** `/q-workspace` creates or repairs a fresh filesystem copy named for the plan directory or ticket slug after final plan review. `thoughts/` is host-owned external storage reached through a symlink, so preserve the link and do not duplicate/sync its plan directory when source and target resolve to the same place. Do not use `git worktree`, symlinks, or hard-link copies: on macOS/APFS use `cp -cRp source-dir clean-copy-dir`; on Linux/GNU coreutils use `cp -a --reflink=auto source-dir clean-copy-dir` (normal-copy fallback when reflinks are unavailable).
