...
- `advanceMode=guided`: default. Continue automatically across graph-safe non-human edges after valid YAML result; stop at explicit human gates. This is the current `auto_mode=false` behavior.
- `advanceMode=autopilot`: continue automatically across graph-safe non-human edges and auto-approve only human gates marked auto-approvable. Never auto-approve final implementation approval or any gate not explicitly marked safe. This is the current `auto_mode=true` behavior.
...
- `.pi/skills/q-design-product/SKILL.md`
