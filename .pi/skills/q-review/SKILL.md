...
The focused review owns every canonical lane artifact beneath `[review_dir]/context/lanes/`. Use the Vamos-owned detached coordinator only: `vamos qrspi lane-run --specs-file <absolute-specs.json> --max-parallel <N>`. Do not call global `subagent(...)`, depend on `pi-subagents`, or use package/default artifact paths.

1. The main reviewer performs a short risk scan of the actual target and chooses the **smallest sufficient review**. A trivial/small scope may use only one targeted reviewer or direct review; add lanes only for a concrete requirement, changed boundary, or safety risk. Do not create a mandatory fan-out or review theater.
1. When selection uncertainty merits a separate pass, preallocate `[review_dir]/context/lane-selection.md`, render the checked-in selector prompt, and run one `qrspi-review-scout` LaneSpec through `lane-run`. Otherwise record the main reviewer's direct, proportional selection in `review.md`.
1. For each selected lane, preallocate a unique Markdown `reportPath`, render its checked-in `q-review/agents/q-review-*.md` prompt with exact evidence scope, and create one parent-owned LaneSpec. Write one absolute specs JSON artifact and run it with a small explicit `--max-parallel` (normally `1`; use more only for independent, material lanes).
1. Validate report containment, regular-file status, required report shape, and concrete evidence before synthesis. A failed lane is an unavailable lane: `lane-run` writes its terminal record and coordinator failure JSONL, while successful peers continue. Record it and verify only its material scope directly; never claim it ran.
1. The stage child alone validates reports, synthesizes, edits planning docs, publishes QRSPI results, and wakes the manager. Lanes never gain graph/result/wake or descendant authority.
...
- Lane Markdown files are embedded prompts, not independently registered agents. Launch only through the Vamos lane coordinator; never fall back to global/package subagent tooling.
- Lane selection is evidence-based, not regex-based. The selector/main reviewer must state why each lane is necessary and why omitted plausible lanes are disproportionate.
- Every lane prompt must demand simplification and concrete evidence. Do not manufacture speculative edge cases, hypothetical failure machinery, or findings unsupported by declared requirements/current code.
- Project guidance, docs health, intent, simplicity, correctness, and safety are review concerns—not automatic lanes. The main reviewer covers them directly when a separate lane would add no material independent value.
- Choose the fewest non-overlapping lanes that cover actual risk; add specialists only for a distinct, concrete question.
