...
Perform a short risk scan of actual changed paths and evidence first. Small/local changes may receive direct review or one targeted lane; add a lane only for a distinct material correctness, safety, guidance, docs, or verification question. If selection is uncertain, run one rendered `q-review-lane-selector.md` prompt as a `qrspi-review-scout` LaneSpec; otherwise document the main reviewer's proportional selection and skipped lanes.

For each selected lane, preallocate a unique report beneath `[review_dir]/context/lanes/`, render its checked-in prompt with exact evidence scope, write parent-owned LaneSpec JSON, and invoke `vamos qrspi lane-run --specs-file <absolute path> --max-parallel <N>`. Use `N=1` unless independent material questions justify concurrency. Validate contained substantive Markdown before synthesis. A failed lane is unavailable and recorded through its terminal record/failure JSONL; successful peers continue, and the main reviewer directly verifies only that failed lane's material scope.
...
1. Run only the proportionate selected lanes through `vamos qrspi lane-run`; read every successful report under `[review_dir]/context/lanes/`; verify candidate findings yourself.
   - Treat empty, raw tool-call, structurally invalid, or evidence-free Markdown as unavailable.
   - Do not retry failed lanes. Record their terminal record/failure JSONL paths and directly verify only the affected material scope.
