...
- Action cards are the first-class manager UX for repairable failures: `state_desync`, `graph_outcome_mismatch`, `workspace_moved`, `active_child_conflict`, `human_gate`, `invalid_child_yaml`, `manual_child_steer`, `superseded_queued_wake`, `pi_compatibility_failed`, `child_launch_failed`, `child_context_exhausted`, `invalid_handoff_artifact`, `handoff_continuation_failed`, `manager_delivery_failed`, and `pending_child_cleanup_failed`.
