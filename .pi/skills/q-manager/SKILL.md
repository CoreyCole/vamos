...
- The CLI, not the manager, launches every graph-safe agent-to-agent transition in guided/autopilot, including same-node handoff resumes. It persists the replacement child before old-pane cleanup, then sends an informational wake with the running stage, pane, and session. The manager never relays a normal transition with `continue`; if a validated graph-safe result arrives without a replacement child, treat it as a CLI failure and run the surfaced diagnostic/recovery action. Discuss mode validates but waits.
- Stop for human gates, blocked/error results, invalid-result retry exhaustion, lock conflict, workspace/safety ambiguity, or ambiguous project judgment named by `docs/q-manager.md`. Every such non-automatic child stop must wake the manager once with child refs and an actionable `steer-child`, `continue`, `doctor`, or recovery command. A live child question is a manager wake even when the Pi process remains interactive and no terminal record exists.
...
- `/q-manager-note` is the explicit exception: load `.pi/skills/q-manager-note/SKILL.md`, record the note as local parent control-plane context, and never forward or embed it in child-visible input or durable artifacts. Apply active manager notes before commands that may auto-launch the next child.
...
1. Stop issuing commands and wait for the child extension/CLI wake. If it reports a running replacement child, relay only its completed stage, running stage, pane, and session; never call `continue` or launch a duplicate. If manager delivery is `compacting`, run the printed `manager-ready` command after parent reset to flush the current wake.
1. For a human gate, blocked/error result, invalid-result retry exhaustion, provider/context failure, ambiguous evidence, launch/cleanup failure, or other non-automatic stop, follow the wake's exact actionable command. Use `steer-child` for human input and keep the same child pane/session. Use `continue`, `doctor`, or a recovery command only when the wake explicitly identifies it as safe. A missing wake for a visible child question is a runtime defect to diagnose, not a reason to wait silently or send raw tmux input.
