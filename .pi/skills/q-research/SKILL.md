...
When the research doc is written, emit this fenced YAML result, followed by the mandatory concise human summary. If invoked as a planning-review helper, defer to `q-research-for-review` and use the helper node stage ID instead of `research`.

Post-YAML natural summary format for this stage: key findings + direct answers to research questions. If multiple key answers, one per line. Caveman speak. Few words. Most important words only. Use:
