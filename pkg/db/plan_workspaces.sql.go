...
const archivePlanWorkspacePrimaryProjects = `-- name: ArchivePlanWorkspacePrimaryProjects :execrows
;
...
func (q *Queries) ArchivePlanWorkspacePrimaryProjects(ctx context.Context, planDirRel string) (int64, error) {
	result, err := q.db.ExecContext(ctx, archivePlanWorkspacePrimaryProjects, planDirRel)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
