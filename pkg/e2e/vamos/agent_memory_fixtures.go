...
func (pages) DensityFixtureThread(threadID string) page {
	return Pages.ThreadWithFixture(threadID, DensityFixtureQueryKey+"="+DensityFixtureQueryValue)
}

func (pages) GroupBubbleFixtureThread(threadID string) page {
	return Pages.ThreadWithFixture(threadID, GroupBubbleFixtureQueryKey+"="+DensityFixtureQueryValue)
}
...
func OpenAgentMemoryThreadFixture(label, fixtureQuery string) spec.Step {
	return customStep("open agent-memory "+label+" fixture thread", func(t testing.TB, ctx *duiruntime.Context) {
		t.Helper()
		visit(t, ctx, Pages.ThreadWithFixture(AgentMemoryFixtureThreadID(), fixtureQuery).Path())
	})
}

func ExpectChatDensityFixtureSeeded() expectation {
	return expectation{customStep("chat density fixture seeded", func(t testing.TB, ctx *duiruntime.Context) {
		t.Helper()
		waitVisible(t, ctx, "#"+DensityReasoningDOMID)
		waitVisible(t, ctx, "#"+DensityTool0DOMID)
		waitVisible(t, ctx, "#"+DensityTool1DOMID)
		assertCount(t, ctx, "#"+DensityReasoningDOMID+"[data-chat-density='reasoning']", 1)
		...
		toolCount, err := ctx.Page.Locator("details[data-chat-density='tool']").Count()
		if err != nil || toolCount < 2 {
			t.Fatalf("expected >=2 tool density details, got %d err=%v", toolCount, err)
		}
		...
		for _, id := range []string{DensityReasoningDOMID, DensityTool0DOMID, DensityTool1DOMID} {
			tag, err := ctx.Page.Locator("#"+id).First().Evaluate("el => el.tagName", nil)
			if err != nil {
				t.Fatal(err)
			}
...
func ExpectGroupBubbleFixtureSeeded() expectation {
	return expectation{customStep("group bubble fixture seeded", func(t testing.TB, ctx *duiruntime.Context) {
		t.Helper()
...
func ExpectBotDMChipPairwiseLinks() expectation {
	return expectation{customStep("BotDMChip exposes pairwise room links", func(t testing.TB, ctx *duiruntime.Context) {
		t.Helper()
		chip := ctx.Page.Locator("#" + GroupBotDMChipDOMID)
		if err := chip.First().WaitFor(); err != nil {
			t.Fatal(err)
		}
		trigger := chip.Locator("[data-slot='dropdown-menu-trigger'], button, [role='button']").First()
